...
通过插件中定义的钩子函数, 来修改代码中的import语法 和 及替换代码内容
...
### **vite运行时插件:**
1. **ensureWatchPlugin**：  
    > build模式 且 开启config.build.watch
   1. 记录可监控的文件
   2. 如果有监控器，则开启监控
...
3. **preAliasPlugin**: 收集未被记录在案的可优化模块  
    1. 判断是否为bare import
    2. 判断是否满足别名替换模式
    3. 再次通过所有vite插件resolveId钩子函数，得到最终的resolveId
    4. 判断resolveId是否为未优化模块
    5. 注册为可优化丢失模块
    6. 执行优化，并收集模块信息
4. **aliasPlugin**：使用rollup下的别名插件  
  根据提供的匹配规则，替换import语法中的别名

5. **prePlugins**：用户自定义的 enforce  为 pre 级别的插件
6. **modulePreloadPolyfillPlugin**：  
    开启config.build.polyfillModulePreload，使得项目开启模块预加载
    1. 在使用import 'vite/modulepreload-polyfill'表达式的模块中，会去执行一次 polyfill 方法
    2. 在 polyfill 方法体内，将会去触发 modulepreload 机制
7. **optimizedDepsBuildPlugin**：build模式
    1.  buildStart钩子：先初始化基础信息
    2. load钩子：读取文件信息 并 确认首次优化是否已执行
    3. transform钩子：等待所有可优化模块已完成，并触发 onCrawlEnd 内置回调
...
9. **resolvePlugin**：对模块地址做正确分析处理，为后续的钩子函数使用
  **resolveId**钩子： 
     1. 收集配置项：scan、isRequest、isFromTsImporter、config.resolve配置项
     2. 如果模块地址为可优化的模块地址，importUrl =  /@fs/xxxFilePath，则xxxFilePath为模块地址
     3. 如果模块地址以/@fs/*开头 -> importUrl =  /@fs/xxxFilePath，并且执行 ★tryFsResolve(xxxFilePath, options)方法：
     4. 如果模块地址以/开头 -> importUrl =  /xxxFilePath 传入 ★tryFsResolve(`${config.root}/xxxFilePath `) 方法
     5. 如果模块地址为相对地址 
     ...
     6. 如果是windows系统，且模块地址以/开头 -> importUrl =  /xxxFilePath
             则会根据importer文件得到所在目录importerPath，执行tryFsResolve(path.resolve(importerPath, "/xxxFilePath"), options)
     7. 如果是windows系统，且模块地址是  importUrl =  C:/xxxFilePath 或者 非windows系统, importUrl = /xxxFilePath
             则直接使用 ★tryFsResolve(importUrl , options)
     8. 如果是外部链接，importUrl = http://xxx.xx.x/module.js，则直接使用该地址
     9. 如果直接就是bare import，则参考9.5中第3和第4步骤
     10. 如果是内置的node模块，importUrl  = node:xxx，则统一替换：
    ...
    **load**钩子：
      ...
      **★tryFsResolve**(importPath, options) 方法：根据url地址，得到file(地址) 和 postfix(查询参数信息)，再根据这些信息去获取文件地址
        ...
        5. 尝试获取file/index.[ext]文件，这里的后缀名可通过config.resolve.extensions来定义，否则默认为 [mjs,js,mts,ts,jsx,tsx,json]
      
      **tryNodeResolve**(importPath, options)方法：尝试以nodejs的方式获取依赖文件地址
      1. 根据传入的importPath，获得 nestedRoot 和 nestedPath
            1.  假如 importPath = foo > bar > xx/baz，结果 nestedRoot  = foo > bar，nestedPath = xx/baz
            2. 假如 importPath = xx/baz，则 nestedRoot = ‘’， nestedPath = xx/baz
      ...
      3. 确定baseDir目录
            1. 根据config.root
            2. 根据importer所在目录
            3. 根据 nestedRoot 和 第1,2步获得的baseDir值，重新确定合适的baseDir  
    ...
    **transform**钩子：
    ...
    3. 执行编译，得到compiledResult
          1. 配置esbuild loader：
             ...
             ② 根据文件后缀名确定  
          2. 配置esbuild tsconfigRaw  
            ① 直接配置 config.esbuild.tsconfigRaw  
            ② 根据tsconfig.json中定义  
          3. 执行esbuild.transform，得到编译结果
    4. 如果 config.esbuild.jsxInject ，则会插入该配置内容
    
13. jsonPlugin：处理json文件内容 转换为 模块代码
    1. 如果配置config.json.stringify，则直接export default JSON.parse(`${JSON.stringify(code)}`)
    2. 否则 dataToEsm 工具函数转成标准的esm格式的文件
...
20. cssPostPlugin：css内容后置处理插件  
    ...
    3. 如果是非serve模式
        1. 如果模块类型是?inline 或 ?html-proxy，则会将css内容记录到内存中，等待插件 vite:build-html 去生产处理
        2. 如果是非?inline，则会将css内容 转换成 模块格式内容
